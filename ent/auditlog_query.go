...
func (_q *AuditLogQuery) QueryAccessRequest() *AccessRequestQuery {
...
func (_q *AuditLogQuery) Clone() *AuditLogQuery {
	...
	return &AuditLogQuery{
		...
		withAccessRequest: _q.withAccessRequest.Clone(),
...
func (_q *AuditLogQuery) WithAccessRequest(opts ...func(*AccessRequestQuery)) *AuditLogQuery {
	query := (&AccessRequestClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withAccessRequest = query
	return _q
}
...
//		GroupBy(auditlog.FieldAccessRequestID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.AuditLog.Query().
//		Select(auditlog.FieldAccessRequestID).
//		Scan(ctx, &v)
...
func (_q *AuditLogQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*AuditLog, error) {
	...
	if query := _q.withAccessRequest; query != nil {
		if err := _q.loadAccessRequest(ctx, query, nodes, nil,
			func(n *AuditLog, e *AccessRequest) { n.Edges.AccessRequest = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *AuditLogQuery) loadAccessRequest(ctx context.Context, query *AccessRequestQuery, nodes []*AuditLog, init func(*AuditLog), assign func(*AuditLog, *AccessRequest)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*AuditLog)
	for i := range nodes {
		fk := nodes[i].AccessRequestID
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "access_request_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *AuditLogQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withAccessRequest != nil {
			_spec.Node.AddColumnOnce(auditlog.FieldAccessRequestID)
		}
	}
